...
public:
	...
	cSurfaceMaterial* m_pWaterMaterial;
...
HRESULT cMyHost::InitDeviceObjects()
{
	...
	pHeightMask = 
		displayManager()
		.imagePool()
		.createResource(cString("height mask"));
	
	pHeightMask->loadResource(
		"media\\textures\\island_mask.dds");

	m_pHeightMap->maskWithImage(pHeightMask);
	safe_release(pHeightMask);
	...
	m_oceanManager.create();

	cRenderMethod* pTempMethod = 
		TheGameHost
		.displayManager()
		.renderMethodPool()
		.createResource("water method");
	pTempMethod->loadEffect(
		cRenderMethod::k_defaultMethod,
		"media\\shaders\\ocean_water.fx");
	...
	m_pWaterMaterial = 
		...
		.displayManager()
		.surfaceMaterialPool()
		.createResource("water material");
	m_pWaterMaterial->setTexture(0, "media\\textures\\ripples.dds");


	pTempMethod->setMaterial(0, m_pWaterMaterial);

	m_oceanManager.setRenderMethod(pTempMethod);
	safe_release(pTempMethod);
	...
	m_pTreeC = 	TheGameHost
		.displayManager()
		.modelPool()
		.createResource("tree type C");
	m_pTreeC->loadResource(
		"media\\models\\big_tree_pmesh.x");


	m_plantManagerC.create(
		&m_terrainSystem,
		m_pTreeC,
		32,
		25.0f,
