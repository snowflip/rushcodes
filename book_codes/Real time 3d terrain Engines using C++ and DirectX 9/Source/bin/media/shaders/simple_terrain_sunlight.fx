...
VS_OUTPUT VS(const VS_INPUT v)
{
  ...
  // transform position immediatly
	...
	float4 viewPos = mul(worldPos, mViewProj);  // position (view space)
...
{   
	...
	float3 texColor0 = tex2D(LinearSamp1, In.vTex1 ); 
	float3 texColor1 = tex2D(LinearSamp2, In.vTex2 ); 
	float3 texColor2 = tex2D(LinearSamp3, In.vTex3 ); 

	// determine the amount of each surface to blend
	float3 Color0 = (texColor0 * BlendControler.r);
	float3 Color1 = (texColor1 * BlendControler.g);
	float3 Color2 = (texColor2 * BlendControler.b);

	// sum the resulting colors 
	// and multiply by the diffuse 
	// vertex color 
	float3 final= ((Color0 + Color1 + Color2)*
...
technique SinglePassTerrain2_0
{
    pass P0
    {
			//FILLMODE = WIREFRAME;
...
technique SinglePassTerrain1_4
{
    pass P0
    {
			//FILLMODE = WIREFRAME;
        ...
        // shaders
        VertexShader = compile vs_1_1 VS();
        PixelShader  = compile ps_1_4 ThreeSurfaceBlend();
    }  
}

technique SinglePassTerrain1_1
{
    pass P0
    {
			//FILLMODE = WIREFRAME;
